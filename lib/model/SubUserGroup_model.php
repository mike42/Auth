...
class SubUserGroup_model {
	...
	*/
	...
	public static function get($parent_group_id, $group_id) {
		$sql = "SELECT * FROM SubUserGroup LEFT JOIN UserGroup ON SubUserGroup.group_id = UserGroup.group_id LEFT JOIN Ou ON UserGroup.ou_id = Ou.ou_id LEFT JOIN ListDomain ON UserGroup.group_domain = ListDomain.domain_id WHERE SubUserGroup.parent_group_id='%s' AND SubUserGroup.group_id='%s'";
